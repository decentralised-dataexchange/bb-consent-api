...
// ServiceLoginUser Implements the user login
func ServiceLoginUser(w http.ResponseWriter, r *http.Request) {
	// Headers
	...
	if err != nil {
		m := fmt.Sprintf("Individual: %v does not exist", lReq.Username)
		common.HandleErrorV2(w, http.StatusBadRequest, m, err)
		return
	}
	tResp := tokenResp{
